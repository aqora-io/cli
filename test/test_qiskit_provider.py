...
def test_qpu_run_uploads_qio_model(monkeypatch: pytest.MonkeyPatch):
...
def test_qpu_run_passes_shots(monkeypatch: pytest.MonkeyPatch):
    mod = load_qiskit_module(FakeClient)
    qpu = mod.QPU()
...
def test_qpu_run_rejects_unsupported_run_options():
    mod = load_qiskit_module(FakeClient)
    qpu = mod.QPU()

    with pytest.raises(NotImplementedError, match="only supports `shots`"):
        qpu.run(mod.QuantumCircuit(1), memory=True)
