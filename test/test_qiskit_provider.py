...
def test_qpu_run_uploads_qio_model(mod, monkeypatch: pytest.MonkeyPatch):
    ...
    # in the signature; omitting the header makes the storage backend reject
    # the PUT with 400.
...
def test_empty_string_job_error_is_not_an_error(mod):
    # The server mirrors the provider's progress message into `error` and
    # returns "" for healthy jobs; only a non-empty error marks failure.
    qpu = mod.QPU()
    job = mod.QPUJob(qpu, "ProviderJob:job-1")
