...
LIST_PROVIDER_PLATFORMS_QUERY = """
query ListProviderPlatforms {
  listProviderPlatforms {
    provider
    name
    id
    vendor
...
class AqoraGraphQLClient:
    ...
    def list_provider_platforms(self) -> list[Mapping[str, Any]]:
        response = _run_sync(lambda: self._client.send(LIST_PROVIDER_PLATFORMS_QUERY))
        return response["listProviderPlatforms"]
