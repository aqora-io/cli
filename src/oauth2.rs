...
pub(crate) struct IssuedTokens {
    ...
    /// The space delimited scopes the authorization server granted.
    #[cfg_attr(not(feature = "extension-module"), allow(dead_code))]
    pub scope: String,
}

impl IssuedTokens {
    fn new(access_token: String, refresh_token: String, expires_in: i64, scope: String) -> Self {
        Self {
            access_token,
            refresh_token,
...
mod tests {
    ...
    const REFRESHED: &str = r#"{"data":{"oauth2Refresh":{"clientError":false,"unauthorized":false,"issued":{"expiresIn":3600,"accessToken":"refreshed-access","refreshToken":"refreshed-refresh","scope":"default"}}}}"#;
