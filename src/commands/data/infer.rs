        ...
        let format = match file_type {
            ...
            FileTypeArg::Csv => {
                ...
                if let Some(delimiter) = self.csv_delimiter {
                    format.delimiter = delimiter.try_into().map_err(|err| {
                        error::user(
                ...
                if let Some(terminator) = self.csv_terminator {
                    format.terminator = Some(terminator.try_into().map_err(|err| {
                        error::user(
                ...
                if let Some(quote) = self.csv_quote {
                    format.quote = quote.try_into().map_err(|err| {
                        error::user(
                ...
                if let Some(escape) = self.csv_escape {
                    format.escape = Some(escape.try_into().map_err(|err| {
                        error::user(
                ...
                if let Some(comment) = self.csv_comment {
                    format.comment = Some(comment.try_into().map_err(|err| {
                        error::user(
                ...
                if let Some(null_regex) = self.csv_null_regex.as_ref() {
                    format.null_regex = Some(Regex::new(null_regex).map_err(|err| {
                        error::user(
                ...
                if let Some(true_regex) = self.csv_true_regex.as_ref() {
                    format.true_regex = Some(Regex::new(true_regex).map_err(|err| {
                        error::user(
                ...
                if let Some(false_regex) = self.csv_false_regex.as_ref() {
                    format.false_regex = Some(Regex::new(false_regex).map_err(|err| {
                        error::user(
