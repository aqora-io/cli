...
use aqora_runner::{
    pipeline::{EvaluationResult, EvaluatorState, Pipeline, PipelineConfig},
...
impl ExperimentSubmission {
    pub async fn load_notebook() -> PyResult<Self> {
        ...
        let evaluated = vec![None; generated_input.len()];
...
impl ExperimentSubmission {
    pub async fn load(&self, index: isize) -> PyResult<()> {
        ...
        let input = self.generated_input.get(index).cloned().ok_or_else(|| {
    ...
    pub async fn finish(&self) -> PyResult<()> {
        ...
        let prev_eval_result = inner.results[index].replace(next_eval_result.clone());
        ...
        if let Some(prev_eval_result) = prev_eval_result {
            ...
            if let Some((next, prev)) = next_metric.zip(prev_metric) {
                let (diff, truthy) = Python::with_gil(|py| {
                    let diff = next
                        .as_ref(py)
                        .call_method1(pyo3::intern!(py, "__sub__"), (prev,))?;
                    let truthy = diff.is_true()?;
                    PyResult::Ok((diff.into_py(py), truthy))
                })?;
                if truthy {
        ...
        if let Some(score) = self
            ...
            .aggregate_vec(Python::with_gil(|py| {
                inner
                    .results
                    .iter()
                    .filter_map(|x| x.clone().map(|x| x.into_py(py)))
                    .collect()
            }))
            .await?
        {
