...
impl UploadPart {
    ...
    {
        let size = body.len();
        let state = PartState::Creating(body, multipart.create_part(client, file, index, size));
        Self {
            client: client.clone(),
            state,
        }
...
where
    M: Multipart,
{
    pub fn new(client: Client, mut multipart: M) -> MultipartUpload<M> {
        let state = UploadState::Creating(multipart.create(&client));
        MultipartUpload {
            client,
            multipart,
            ...
            match state {
                ...
                Uploading { file, parts } => {
                    ...
                    if pending {
                        ...
                        *state = Completing(self.multipart.complete(&self.client, file, etags));
                    } else {
